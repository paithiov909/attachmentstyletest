  ...
  <div id="wrapper">
    ...
    <div id="top" class="container">
      <div class="jumbotron">
        <h1>愛着スタイル診断テスト</h1>
        <p>
          <a href="https://g.co/kgs/aRxumE" target="_blank" rel="noopener">回避性愛着障害～絆が稀薄な人たち～ (光文社新書)</a>
          をもとに作成した愛着スタイル診断テストです。次の質問に対し、過去数年間のご自分の傾向を思い浮かべながら、もっともあてはまる選択肢を選んでください。
        </p>
        <h2><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>&nbsp;テストするにあたって</h2>
        <p>
          この診断テストは、自分の愛着スタイルについて、あなた自身で考えてみる手がかりにしてもらうためのものです。<wbr />
          このテストの結果だけでは、専門家の支援が必要な「愛着障害」や、その他のこころの問題を抱えているかどうかを正確に判断することはできません。
        </p>
        <p>
      ...
      <div class="container">
        <h4>
          <a
            ...
            role="button"
            @click.prevent="calcResult"
            >診断する</a
          >
        </h4>
        <p v-show="validation">回答していない質問があります。すべての質問に回答してください。</p>
      </div>
    </div>
    <!-- End #contact -->
    ...
    <div id="education">
      <div class="container">
        ...
        <div class="education-block add-margin">
...
export default {
  ...
  computed: {
    projects() {
      return [
        {
          img:
            "https://images-fe.ssl-images-amazon.com/images/I/31PfyK01o%2BL._SL160_.jpg",
          title: "回避性愛着障害 絆が稀薄な人たち (光文社新書)",
          info: "岡田 尊司",
          link:
            "http://www.amazon.co.jp/exec/obidos/ASIN/4334037755//ref=nosim/",
        },
        {
          img:
          ...
          info: "岡田 尊司",
          link:
            "http://www.amazon.co.jp/exec/obidos/ASIN/4334036430//ref=nosim/",
        },
        {
          img:
            "https://images-fe.ssl-images-amazon.com/images/I/510NOAWnjKL._SL160_.jpg",
          title: "生きるのが面倒くさい人 回避性パーソナリティ障害 (朝日新書)",
          info: "岡田 尊司",
          link:
            "http://www.amazon.co.jp/exec/obidos/ASIN/4022736704//ref=nosim/",
        },
      ];
    },
    quickchart() {
      ...
      return `https://quickchart.io/chart?bkg=white&c=${JSON.stringify({
        ...
        data: {
          labels: [
            encodeURIComponent("安定型愛着スコア"),
            encodeURIComponent("不安型愛着スコア"),
            encodeURIComponent("回避型愛着スコア"),
            encodeURIComponent("未解決型愛着スコア"),
          ],
          datasets: [{ data: [score.scA, score.scB, score.scC, score.scD] }],
        },
      })}`;
    },
  },
  watch: {
    score(newscore, oldscore) {
      ...
      if (newscore.scA >= 15) {
        $("#antei").addClass("warning");
      } else if (newscore.scA >= 10) {
        $("#antei").addClass("danger");
      }
      if (newscore.scB >= 15) {
        $("#fuan").addClass("warning");
      } else if (newscore.scB >= 10) {
        $("#fuan").addClass("danger");
      }
      if (newscore.scC >= 15) {
        $("#kaihi").addClass("warning");
      } else if (newscore.scC >= 10) {
        $("#kaihi").addClass("danger");
      }
      if (newscore.scD >= 10) {
        $("#mikai").addClass("warning");
      } else if (newscore.scD >= 5) {
        $("#mikai").addClass("danger");
      }
    },
  },
  methods: {
